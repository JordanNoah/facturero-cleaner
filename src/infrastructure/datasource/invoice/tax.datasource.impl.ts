import { TaxDatasource } from "../../../domain/datasource/invoice/tax.datasource";
import { v4 } from "uuid"
...
export default class TaxDatasourceImpl extends TaxDatasource {
    ...
    async getTaxesByDetailId(detailId: number): Promise<any> {
        try {
            const taxes = await TaxSequelize.findAll({
                where:{
                    detailId: detailId
                }
            })

            return taxes.map(tax => TaxEntity.create(tax))
        } catch (error) {
